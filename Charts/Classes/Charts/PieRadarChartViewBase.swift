...
@objc
public protocol PieChartAnimationStopDelegate
{
    /// Called when a value has been selected inside the chart.
    /// - parameter entry: The selected Entry.
    /// - parameter dataSetIndex: The index in the datasets array of the data object the Entrys DataSet is in.
    optional func pieChartAnimationStop(rotationAngle:CGFloat)
    optional func pieChartRotating()
}

/// Base class of PieChartView and RadarChartView.
public class PieRadarChartViewBase: ChartViewBase
{
    public weak var stopDelegate: PieChartAnimationStopDelegate?
    ...
    @objc private func decelerationLoop()
    {
        ...
        if(abs(_decelerationAngularVelocity) < 1)
        {
            stopDelegate!.pieChartAnimationStop?(self.rotationAngle)
            stopDeceleration()
        }
    }
